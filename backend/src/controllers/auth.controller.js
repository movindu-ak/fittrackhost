...
export const register = async (req, res) => {
  try {
    ...
    // convert string to qr
    const qrImage  = await qrcode.toDataURL(qrString);
    await User.updateOne({ _id: user._id }, { qrCode: qrString });

    console.log('✅ User created with QR:', user._id, qrString);
...
// @desc    Register trainer (admin only)
...
export const getAdminStats = async (req, res) => {
  try {
    ...
    const [memberCount, todayRevenue, membershipDistributionData, monthlyRevenueRaw] = await Promise.all([
      ...
      Payment.aggregate([
        { $match: { status: 'captured', createdAt: { $gte: startOfDay, $lte: endOfDay } } },
      ...
      Payment.aggregate([
        ...
        { $group: { _id: { year: { $year: '$capturedAt' }, month: { $month: '$capturedAt' } }, revenue: { $sum: '$amount' } } },
...
// @desc    Get all members (paginated + searchable)
