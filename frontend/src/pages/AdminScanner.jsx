...
export default function AdminScanner() {
  ...
  // ── fetch today's list on mount and every 30s ──────────
  useEffect(() => {
    fetchToday();
  ...
  const startScanner = () => {
    ...
    scanner.render(
      (decoded) => { stopScanner(); handleScan(decoded); },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6">
              ...
              {scanning && (
                <div id="qr-reader" className="rounded-xl overflow-hidden mb-5" />
              )}
                  ...
                  {result.ok && result.attendance && (
                    <div className="mt-2 text-sm space-y-0.5 text-emerald-400">
                      ...
                      {result.attendance.durationText && (
                        <p>⏱ Duration: {result.attendance.durationText}</p>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
              <h3 className="font-bold text-emerald-400 mb-3 flex items-center gap-2">
                ...
                Inside Gym ({todayData?.checkedInCount ?? 0})
              </h3>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {todayData?.checkedIn?.length ? (
                  todayData.checkedIn.map(m => (
                  ...
                  <p className="text-sm text-gray-600 text-center py-4">No members currently inside</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5">
              <h3 className="font-bold text-orange-400 mb-3">
                Checked Out ({todayData?.checkedOutCount ?? 0})
              </h3>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {todayData?.checkedOut?.length ? (
                  todayData.checkedOut.map(m => (
                    <div key={m._id} className="bg-orange-950 border border-orange-800 rounded-lg px-3 py-2">
                      ...
                      <p className="text-xs text-orange-600">
                        {m.entryTime} → {m.exitTime} · {m.durationText}
                      </p>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-600 text-center py-4">No check-outs yet today</p>
                )}
              </div>
            </div>
